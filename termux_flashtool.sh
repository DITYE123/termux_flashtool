...
version=$(curl --connect-timeout 2 --max-time 2 "https://github.com/DITYE123/termux_flashtool/blob/main/version?raw=true" 2>/dev/null)
if [ $? -eq 0 ]
then
if [ ${version} = "1.2" ]
then
echo -e "\033[42m"当前为最新版本"\033[0m"
else
echo 检测到有可用更新
fi
else
echo -e "\033[41m"网络连接错误，检测更新失败"\033[0m"
fi
